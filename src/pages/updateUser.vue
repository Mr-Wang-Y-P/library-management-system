...
import { getUserInfo,updateUser } from '../../server/data/getUser'
...
onMounted(async () => {
    ...
    ruleForm.image = res.data[0].image
    ruleForm.name = res.data[0].name
    ruleForm.phone = res.data[0].phone
    ruleForm.password = res.data[0].password
    ruleForm.email = res.data[0].email
    // getUserData.value = res.data[0]
    await nextTick(() => {
...
const resetForm =async (formEl) => {
    ...
    const res = await getUserInfo(route.params.id)
    getUserData.value = res.data[0]
    ruleForm.url = getUserData.value.url
    ruleForm.name = getUserData.value.name
    ruleForm.phone = getUserData.value.phone
    ruleForm.password = getUserData.value.password
    ruleForm.email = getUserData.value.email
}
...
const submitForm = async (formEl) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      try {
        let userValue = []
        ruleForm.userId = Number((route.params.id))
        userValue = JSON.parse(JSON.stringify(ruleForm))
        const res = await updateUser(userValue);
        if (res.status === 200) {
